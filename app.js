/* ============================================================
   FOLIO — Portfolio Builder (Updated with Pricing, Export, Contact)
   ============================================================ */
...
const planLimits = {
  basic: { projects: 3, certificates: 2, templates: 'basic', export: false, analytics: false },
  pro: { projects: 10, certificates: 10, templates: 'standard', export: true, analytics: false },
  premium: { projects: 999, certificates: 999, templates: 'all', export: true, analytics: true }
};

function loadState() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return { users: [], session: null, portfolios: {}, theme: 'dark', messages: {} };
    return JSON.parse(raw);
  } catch (e) { return { users: [], session: null, portfolios: {}, theme: 'dark', messages: {} }; }
}
...
function register(data) {
  ...
  const user = { id: 'u_' + Math.random().toString(36).slice(2, 11), ...data, password: btoa(data.password), plan: 'basic', createdAt: new Date().toISOString() };
...
function topNav(active = '') {
  const links = [{ href: '#/', label: 'Home', key: 'home' }, { href: '#/pricing', label: 'Pricing', key: 'pricing' }, { href: '#/demo', label: 'Demo', key: 'demo' }, { href: '#/login', label: 'Sign in', key: 'login' }];
...
function renderPricing(app) {
  const user = currentUser();
  const plans = [
    { name: 'Basic', price: 5, desc: 'Unlocks ~25% of platform features. Perfect for getting started.', features: [{ text: 'Basic portfolio customization', included: true }, { text: 'Limited templates', included: true }, { text: 'Up to 3 Projects', included: true }, { text: 'Up to 2 Certificates', included: true }, { text: 'Basic contact section', included: true }, { text: 'PDF/Image Export', included: false }, { text: 'Advanced Analytics', included: false }] },
    { name: 'Pro', price: 15, desc: 'Unlocks ~50% of features. More freedom to build and share.', features: [{ text: 'More customization options', included: true }, { text: 'More templates', included: true }, { text: 'Up to 10 Projects', included: true }, { text: 'Up to 10 Certificates', included: true }, { text: 'Additional portfolio sections', included: true }, { text: 'PDF & Image Export', included: true }, { text: 'Advanced Analytics', included: false }] },
    { name: 'Premium', price: 18, desc: 'Unlocks 100% of all features. The ultimate portfolio toolkit.', recommended: true, features: [{ text: 'Unlimited Projects', included: true }, { text: 'Unlimited Certificates', included: true }, { text: 'Full customization', included: true }, { text: 'Premium templates', included: true }, { text: 'All portfolio sections', included: true }, { text: 'PDF & Image Export', included: true }, { text: 'Advanced Analytics (demo)', included: true }] }
  ];

  app.innerHTML = `<div class="min-h-screen relative overflow-x-hidden">${topNav('pricing')}<section class="relative pt-32 pb-16 px-6 overflow-hidden"><div class="hero-grid"></div><div class="orb" style="width:500px;height:500px;background:#ff6b4a;top:-150px;right:-150px;opacity:.25;"></div><div class="max-w-5xl mx-auto relative text-center"><div class="text-xs font-mono mb-3 animate-fade-in" style="color:var(--accent);">// PRICING</div><h1 class="text-4xl md:text-6xl font-extrabold tracking-tight leading-[1.05] animate-fade-in" style="animation-delay:.1s;">Plans that scale with<br/><span class="font-serif italic font-normal" style="color:var(--accent);">your ambition.</span></h1><p class="mt-6 text-lg max-w-2xl mx-auto animate-fade-in" style="color:var(--text-muted);animation-delay:.2s;">Start free, upgrade when you need more. No hidden fees, cancel anytime.</p></div></section><section class="pb-24 px-6"><div class="max-w-6xl mx-auto grid md:grid-cols-3 gap-6 items-start">${plans.map(plan => `<div class="card pricing-card p-8 relative ${plan.recommended ? 'recommended' : ''}">${plan.recommended ? '<div class="recommended-badge">Recommended</div>' : ''}<h3 class="text-lg font-bold tracking-tight">${plan.name}</h3><div class="mt-4 flex items-baseline gap-1"><span class="text-4xl font-extrabold">€${plan.price}</span><span class="text-sm" style="color:var(--text-muted);">/month</span></div><p class="mt-2 text-sm" style="color:var(--text-muted);">${plan.desc}</p><button onclick="selectPlan('${plan.name.toLowerCase()}')" class="btn ${plan.recommended ? 'btn-primary' : 'btn-secondary'} w-full mt-6" ${user && user.plan === plan.name.toLowerCase() ? 'disabled' : ''}>${user && user.plan === plan.name.toLowerCase() ? 'Current Plan' : 'Choose ' + plan.name}</button><div class="mt-8 space-y-3">${plan.features.map(f => `<div class="flex items-center gap-3 text-sm"><i class="fas ${f.included ? 'fa-circle-check' : 'fa-circle-xmark'}" style="color:${f.included ? 'var(--success)' : 'var(--text-dim)'};"></i><span style="color:${f.included ? 'var(--text)' : 'var(--text-dim)'};">${f.text}</span></div>`).join('')}</div></div>`).join('')}</div></section>${footer()}</div>`;
}

function selectPlan(planName) {
  if (!currentUser()) { toast('info', 'Please sign in', 'You need an account to select a plan.'); location.hash = '#/login'; return; }
  const user = currentUser();
  user.plan = planName;
  saveState();
  toast('success', 'Plan updated', `You are now on the ${planName.charAt(0).toUpperCase() + planName.slice(1)} plan.`);
...
async function exportPortfolio(format, username) {
  ...
  iframe.onload = async () => {
    try {
      ...
      const element = doc.getElementById('public-portfolio-content');
      if (!element) throw new Error('Content not found');
      
      const canvas = await html2canvas(element, { backgroundColor: appState.theme === 'dark' ? '#08080a' : '#fafafa', scale: 2, useCORS: true });
      ...
      toast('success', 'Export complete', `Your portfolio has been downloaded.`);
    } catch (e) {
      console.error(e);
      toast('error', 'Export failed', 'An error occurred during rendering.');
    } finally {
...
function renderEdit(app) {
  ...
  const certLocked = p.certificates.length >= limits.certificates;
  const projLocked = p.projects.length >= limits.projects;

  app.innerHTML = dashboardLayout('edit', `<div class="mb-6"><h1 class="text-3xl font-bold tracking-tight">Edit your portfolio</h1><p class="mt-1" style="color:var(--text-muted);">Changes save automatically. Use the sections below to build your story.</p></div><div class="sticky top-0 z-20 -mx-6 px-6 py-3 mb-6 flex gap-1 overflow-x-auto scrollbar-hide" style="background:var(--bg);border-bottom:1px solid var(--border);">${['Profile', 'About', 'Skills', 'Projects', 'Experience', 'Education', 'Certificates', 'Social', 'Contact', 'Resume'].map((s, i) => `<a href="#section-${s.toLowerCase()}" class="tab ${i === 0 ? 'active' : ''}" onclick="setActiveTab(this)">${s}</a>`).join('')}</div><div class="space-y-6">${renderProfileSection(p)}${renderAboutSection(p)}${renderSkillsSection(p)}${renderProjectsSection(p, projLocked)}${renderExperienceSection(p)}${renderEducationSection(p)}${renderCertificatesSection(p, certLocked)}${renderSocialSection(p)}${renderContactSection(p)}${renderResumeSection(p)}</div><div class="mt-8 flex flex-wrap gap-3 justify-between items-center card p-4 sticky bottom-4" style="box-shadow:0 -8px 24px -8px rgba(0,0,0,.4);"><div class="text-xs" style="color:var(--text-muted);"><i class="fas fa-circle-check" style="color:var(--success);"></i> All changes saved automatically</div><div class="flex gap-2"><a href="#/portfolio" class="btn btn-secondary btn-sm"><i class="fas fa-eye"></i> Preview</a><a href="#/u/${currentUser().username}" target="_blank" class="btn btn-primary btn-sm"><i class="fas fa-arrow-up-right-from-square"></i> View public</a></div></div>`);
}
    ...
    <nav class="portfolio-nav fixed top-0 inset-x-0 z-40">
      <div class="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <a href="#/register" class="btn btn-primary btn-sm hidden sm:flex">Build your own</a>
        </div>
      </div>
    </nav>
    
    <div id="public-portfolio-content" class="pt-24">
      <div class="max-w-3xl mx-auto px-6 py-12">
        ...
        <section class="mb-16 reveal">
          <h2 class="text-xs font-mono mb-6" style="color:var(--accent);">// SEND A MESSAGE</h2>
          <div class="card p-8">
            <form id="contact-form" onsubmit="submitContactForm(event, '${user.id}')" class="grid md:grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input type="tel" name="phone" class="input" placeholder="+1 (555) 000-0000" />
              </div>
              <div>
                <label class="label">Subject (Optional)</label>
                <input type="text" name="subject" class="input" placeholder="Job Opportunity" />
              </div>
              <div class="md:col-span-2">
                <label class="label">Message</label>
                <textarea name="message" class="textarea" required placeholder="Hi Alex, I'd love to talk about..." style="min-height: 140px;"></textarea>
              </div>
...
function submitContactForm(e, ownerId) {
  e.preventDefault();
  const data = Object.fromEntries(new FormData(e.target));
  if (!data.name || !data.email || !data.message) {
    return toast('error', 'Missing fields', 'Please fill in name, email, and message.');
  }
  if (!appState.messages) appState.messages = {};
  if (!appState.messages[ownerId]) appState.messages[ownerId] = [];
  appState.messages[ownerId].push({ ...data, date: new Date().toISOString() });
  saveState();
  e.target.reset();
  toast('success', 'Message sent', 'Your message has been delivered to the portfolio owner.');
}
...
function addProject() { const p = currentPortfolio(); const limits = planLimits[currentUser().plan]; if (p.projects.length >= limits.projects) { showLockMessage(); return; } p.projects.push({ title: '', description: '', image: '', link: '', tags: [] }); saveState(); renderEdit(document.getElementById('app')); document.getElementById('section-projects').scrollIntoView(); }
...
function addCertificate() { const p = currentPortfolio(); const limits = planLimits[currentUser().plan]; if (p.certificates.length >= limits.certificates) { showLockMessage(); return; } p.certificates.push({ name: '', issuer: '', date: '', link: '#' }); saveState(); renderEdit(document.getElementById('app')); document.getElementById('section-certificates').scrollIntoView(); }
